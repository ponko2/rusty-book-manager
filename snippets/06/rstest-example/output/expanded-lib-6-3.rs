    ...
    pub mod __任意のメソッド名 {
        ...
        impl Matcher {
            ...
            fn matches(&self, x: &u32) -> bool {
                match self {
                    ...
                    Matcher::Pred(__mockall_pred) => [__mockall_pred.0.eval(x)]
                        .iter()
        ...
        impl ::std::fmt::Display for Matcher {
            fn fmt(&self, __mockall_fmt: &mut ::std::fmt::Formatter<'_>) -> ::std::fmt::Result {
                match self {
                    Matcher::Always => __mockall_fmt.write_fmt(format_args!("<anything>")),
                    Matcher::Func(_) => __mockall_fmt.write_fmt(format_args!("<function>")),
                    Matcher::FuncSt(_) => {
                        __mockall_fmt.write_fmt(format_args!("<single threaded function>"))
                    }
        ...
        impl Common {
            fn call(&self, desc: &str) {
                self.times.call().unwrap_or_else(|m| {
                    let desc = {
                        let res =
                            ::alloc::fmt::format(format_args!("{0}", self.matcher.lock().unwrap()));
                        res
                    };
                    {
                        ::std::rt::panic_fmt(format_args!(
                            "{0}: Expectation({1}) {2}",
                            "Mock任意のトレイト名::任意のメソッド名", desc, m,
                        ));
                    };
                });
                ...
                self.rfunc
                    ...
                    .unwrap_or_else(|message| {
                        ...
                        };
                        {
                            ::std::rt::panic_fmt(format_args!(
                                "{0}: Expectation({1}) {2}",
                                "Mock任意のトレイト名::任意のメソッド名", desc, message,
                            ));
                        };
                    })
            }
        ...
        impl Expectations {
            ...
            pub(in super::super) fn call(&self, x: u32) -> Option<u32> {
                self.0
                    .iter()
                    .find(|__mockall_e| {
                        __mockall_e.matches(&x) && (!__mockall_e.is_done() || self.0.len() == 1)
                    })
...
impl 任意のトレイト名 for Mock任意のトレイト名 {
    fn 任意のメソッド名(&self, x: u32) -> u32 {
        let no_match_msg = {
            let res = ::alloc::fmt::format(format_args!("{0}: No matching expectation found", {
                let res = ::alloc::fmt::format(format_args!(
                    "Mock任意のトレイト名::任意のメソッド名({0:?})",
                    ::mockall::MaybeDebugger(&x),
                ));
